...
export const getOrigins = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error al obtener origenes", error });
  }
};
  ...
  if (!country || !region || !climate || !description) {
    return res
      ...
      .json({
        ...
        data: { country, region, climate, description },
      });
  }
...
export const updateOrigin = async (req, res) => {
  ...
  try {
    const updatedOrigin = await Origin.findByIdAndUpdate(
      id,
      { country, region, climate, description },
      { new: true }
    );
    ...
    if (!updatedOrigin) {
      ...
      return res
        .status(200)
        .json({ message: "Origen actualizado", data: updatedOrigin });
    }
  } catch (error) {
    console.error("Error details:", error); // Agregar detalles del error
    // Manejo de errores de validación de Mongoose
...
export const deleteOrigin = async (req, res) => {
  ...
  try {
    ...
    if (!origin) {
      ...
      return res
        .status(200)
        .json({ message: "Origen eliminado", data: origin });
    }
  } catch (error) {
    console.error(error);
